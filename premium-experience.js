(() => {
  ...
  function demoRecentItems() {
    const item = currentItem();
    return [
      {title:item?.title || 'La Vie En Rose', artist:item?.artist || 'Grace Jones', album:item?.album || 'Portfolio', played_at:new Date().toISOString(), play_uri:item?.uri || 'library:track:demo', is_radio:isRadioMode()},
      {title:'Teardrop',artist:'Massive Attack',album:'Mezzanine',play_uri:'library:track:42',played_at:new Date(Date.now()-18*60000).toISOString()},
      {title:'Radio Beograd 202',artist:'Live radio',play_uri:'library:playlist:13',is_radio:true,played_at:new Date(Date.now()-42*60000).toISOString()},
      {title:'Riders on the Storm',artist:'The Doors',album:'L.A. Woman',play_uri:'library:track:43',played_at:new Date(Date.now()-64*60000).toISOString()},
      {title:'Moon Safari',artist:'Air',album:'Moon Safari',play_uri:'library:album:6',played_at:new Date(Date.now()-93*60000).toISOString()},
      {title:'Naxi Radio',artist:'Live radio',play_uri:'library:playlist:11',is_radio:true,played_at:new Date(Date.now()-130*60000).toISOString()},
    ];
  }
  ...
  async function refreshMiniQueue() {
    ...
    const item = currentItem();
    if (items.length) {
      body.innerHTML = items.slice(0,3).map((item,index) => `<div class="desktop-mini-queue-row"><span>${String(index+1).padStart(2,'0')}</span><span><b>${escapeHtml(item.title || 'Untitled')}</b><small>${escapeHtml(item.artist || item.album || 'OwnTone')}</small></span></div>`).join('');
    } else if (item) {
