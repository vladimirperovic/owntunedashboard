(() => {
  ...
  function readSource() {
    try { return JSON.parse(sessionStorage.getItem('owntone-playing-source') || 'null'); }
    catch (_) { return null; }
  }
  function writeSource(source) {
    currentSource = source;
    try { sessionStorage.setItem('owntone-playing-source', JSON.stringify(source)); } catch (_) {}
    syncPlayingFrom();
  }
  function appState() { return app()?.state || {}; }
  function currentItem() { return appState().current || null; }
  function isRadioMode() { return document.body.classList.contains('radio-mode'); }
  function isDemo() { return !!appState().demo; }
  function apiUrl(path) { return `${apiBase}${path.startsWith('/') ? path : '/' + path}`; }
  function schedulerUrl(path) { return `${schedulerBase}${path.startsWith('/') ? path : '/' + path}`; }
  ...
  function inferSource() {
    ...
    if (isRadioMode()) return {kind:'radio', label:item?.title || 'Live radio'};
  ...
  function capturePlaybackSource(event) {
    ...
    if (radio) {
      writeSource({kind:'radio', label:radio.querySelector('.radio-station-name, b')?.textContent?.trim() || 'Radio'});
      return;
    }
    const album = target.closest('.album-card[data-uri]');
    if (album) {
      writeSource({kind:'album', label:album.querySelector('.album-copy b')?.textContent?.trim() || 'Album'});
      return;
    }
    const playlist = target.closest('.playlist-card[data-uri]');
    if (playlist) {
      writeSource({kind:'playlist', label:playlist.querySelector('.playlist-copy b')?.textContent?.trim() || 'Playlist'});
      return;
    }
    const quick = target.closest('.quick-card');
    if (quick) {
      writeSource({kind:'playlist', label:quick.querySelector('b')?.textContent?.trim() || quick.dataset.playlist || 'Quick access'});
      return;
    }
  ...
  async function openTrackSheet() {
    ...
    const source = inferSource();
    const rows = [
      ['Artist', item?.artist], ['Album', item?.album], ['Year', item?.year], ['Format', String(item?.type || '').toUpperCase()],
      ['Bitrate', item?.bitrate], ['Sample rate', item?.samplerate ? `${item.samplerate} Hz` : ''], ['Source', source.label],
      ['Path', item?.path],
    ].filter(([,value]) => value !== undefined && value !== null && String(value).trim());
    const lyrics = String(item?.lyrics || '').trim();
    openSheet('Track details', 'NOW PLAYING', `
  ...
  function mountOutputPicker() {
    ...
    new MutationObserver(syncOutputButton).observe(select, {childList:true, subtree:true, attributes:true});
  }
  ...
  function openOutputSheet() {
    ...
    const rows = [...(select?.options || [])].filter(option => option.value).map(option => {
    ...
    openSheet('AirPlay output', 'OUTPUT', `
    ...
    $('premiumSheetBody').querySelectorAll('[data-output-id]').forEach(button => button.addEventListener('click', () => {
      if (!select) return;
    ...
    volumeInput?.addEventListener('input', () => {
      ...
      if (main) { main.value = String(value); main.dispatchEvent(new Event('input', {bubbles:true})); }
    });
  ...
  function enhanceAlbumCards() {
    document.querySelectorAll('.album-card[data-uri]').forEach(card => {
      ...
      button.setAttribute('aria-label', `Album details: ${card.querySelector('.album-copy b')?.textContent || 'album'}`);
      button.innerHTML = icons.info;
  ...
  async function openAlbumDialog(card) {
    ...
    inner.querySelector('[data-album-action="play"]').addEventListener('click', () => { writeSource({kind:'album',label:name}); app()?.playUri?.(uri); albumDialog.close(); });
    inner.querySelector('[data-album-action="shuffle"]').addEventListener('click', () => { writeSource({kind:'album',label:`${name} · shuffled`}); app()?.playUri?.(uri,{shuffle:true}); albumDialog.close(); });
    inner.querySelector('[data-album-action="queue"]').addEventListener('click', () => addAlbumToQueue(uri, name));
    if (typeof albumDialog.showModal === 'function') albumDialog.showModal(); else albumDialog.setAttribute('open','');
    await loadAlbumTracks(name, artist);
  }

  async function addAlbumToQueue(uri, name) {
    if (!uri) return;
    if (isDemo()) { toast(`${name} added to preview queue`); return; }
    try {
      ...
      refreshMiniQueue();
    } catch (error) { toast(`Queue failed: ${error.message}`); }
  }
  ...
  async function loadAlbumTracks(name, artist) {
    ...
    if (isDemo()) {
      ...
      list.innerHTML = demoTracks.map((title,index) => `<div class="album-track-row"><span>${String(index+1).padStart(2,'0')}</span><span><b>${escapeHtml(title)}</b><small>${escapeHtml(artist)}</small></span><em>${3 + (index%3)}:${String(8 + index*7).padStart(2,'0')}</em></div>`).join('');
    ...
    try {
      ...
      list.innerHTML = tracks.length ? tracks.map((track,index) => `<div class="album-track-row"><span>${String(index+1).padStart(2,'0')}</span><span><b>${escapeHtml(track.title || 'Untitled')}</b><small>${escapeHtml(track.artist || artist)}</small></span><em>${escapeHtml(track.track_number || '')}</em></div>`).join('') : '<div class="premium-empty">Tracklist is not available from OwnTone search.</div>';
    } catch (error) {
  ...
  function demoRecentItems() {
    const item = currentItem();
    return [
      {title:item?.title || 'La Vie En Rose', artist:item?.artist || 'Grace Jones', album:item?.album || 'Portfolio', played_at:new Date().toISOString(), play_uri:item?.uri || '', is_radio:isRadioMode()},
      {title:'Teardrop',artist:'Massive Attack',album:'Mezzanine',played_at:new Date(Date.now()-18*60000).toISOString()},
      {title:'Radio Beograd 202',artist:'Live radio',is_radio:true,played_at:new Date(Date.now()-42*60000).toISOString()},
      {title:'Riders on the Storm',artist:'The Doors',album:'L.A. Woman',played_at:new Date(Date.now()-64*60000).toISOString()},
      {title:'Moon Safari',artist:'Air',album:'Moon Safari',played_at:new Date(Date.now()-93*60000).toISOString()},
      {title:'Naxi Radio',artist:'Live radio',is_radio:true,played_at:new Date(Date.now()-130*60000).toISOString()},
    ];
  }
  ...
  async function refreshRecentRail() {
    ...
    if (isDemo()) items = demoRecentItems();
      ...
      try {
        ...
        items = data?.items || [];
      } catch (_) { items = demoRecentItems().slice(0,1); }
    }
    recentRail.innerHTML = items.length ? items.slice(0,8).map((item,index) => {
      const title = item.title || item.station_name || 'Unknown';
      const sub = item.artist || item.station_name || item.album || (item.is_radio ? 'Live radio' : 'OwnTone');
    ...
    }).join('') : '<div class="premium-empty">No listening history yet.</div>';
    recentRail.querySelectorAll('[data-uri]').forEach(button => button.addEventListener('click', () => {
      writeSource({kind:'history', label:'Recently played'});
  ...
  function mountMiniQueue() {
    ...
    miniQueue.querySelector('.premium-mini-queue-open').addEventListener('click', () => $('queueDrawerButton')?.click());
    refreshMiniQueue();
  }
  ...
  async function refreshMiniQueue() {
    ...
    if (isDemo()) {
      items=[
        {title:'Teardrop',artist:'Massive Attack'}, {title:'Riders on the Storm',artist:'The Doors'}, {title:'Roads',artist:'Portishead'}
      ];
    } else {
      try {
        const now = await requestJson(apiUrl('/queue?id=now_playing'), {cache:'no-store'});
        const position = Number(now?.items?.[0]?.position ?? 0);
        const data = await requestJson(apiUrl(`/queue?start=${position+1}&end=${position+4}`), {cache:'no-store'});
        items = data?.items || [];
      } catch (_) { items=[]; }
    }
    body.innerHTML = items.length ? items.slice(0,3).map((item,index) => `<div class="desktop-mini-queue-row"><span>${String(index+1).padStart(2,'0')}</span><span><b>${escapeHtml(item.title || 'Untitled')}</b><small>${escapeHtml(item.artist || item.album || 'OwnTone')}</small></span></div>`).join('') : '<div class="premium-empty compact">Queue is empty.</div>';
  }
  ...
  function mountFullscreen() {
    ...
    fullscreen.addEventListener('click', event => { if (event.target === fullscreen) fullscreen.close(); });
  ...
  function openFullscreen() {
    ...
    if (typeof fullscreen.showModal === 'function') fullscreen.showModal(); else fullscreen.setAttribute('open','');
  }
  ...
  function syncFullscreen() {
    ...
    $('fullscreenOutputName').textContent = $('outputSelect')?.options[$('outputSelect')?.selectedIndex]?.text || 'No output';
    const src = $('artwork')?.getAttribute('src') || '';
    const img = $('fullscreenArtwork');
    const fallback = $('fullscreenArtFallback');
    if (src) { if (img.getAttribute('src') !== src) img.src = src; img.hidden=false; fallback.hidden=true; }
    ...
    const playing = player.state === 'play' || String($('playButton')?.getAttribute('aria-label') || '').toLowerCase().includes('pause');
    const play = fullscreen.querySelector('.fullscreen-play');
    if (play.dataset.state !== (playing ? 'pause':'play')) { play.dataset.state = playing ? 'pause':'play'; play.innerHTML = playing ? icons.pause : icons.play; play.setAttribute('aria-label', playing ? 'Pause':'Play'); }
  }
  ...
  async function syncArtworkMood() {
    ...
    if (src && art?.complete && art.naturalWidth) {
      try {
        ...
        for (let i=0;i<data.length;i+=16) { if (data[i+3] < 160) continue; r+=data[i]; g+=data[i+1]; b+=data[i+2]; count++; }
        if (count) {
          r=Math.round(r/count); g=Math.round(g/count); b=Math.round(b/count);
          const max=Math.max(r,g,b), min=Math.min(r,g,b);
          if (max-min < 35) { r=Math.min(235,r+35); g=Math.max(55,g-12); b=Math.max(45,b-20); }
    ...
    let hash=0; for (const ch of text) hash=((hash<<5)-hash+ch.charCodeAt(0))|0;
  ...
  function mountMutationObservers() {
    ...
    if (player) new MutationObserver(() => { syncPremiumNowPlaying(); enhanceAlbumCards(); }).observe(player, {subtree:true, childList:true, characterData:true, attributes:true, attributeFilter:['src','class','aria-label']});
