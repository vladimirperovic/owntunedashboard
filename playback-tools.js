  ...
  function renderQueue(items, total, currentId) {
    ...
    if (!items.length) {
      if (current) {
        ...
        body.innerHTML = `<div class="queue-list"><div class="queue-row is-playing live-only" data-id="${escapeHtml(current.id)}">
          <span class="queue-grip">${icons.grip}</span>
          <span class="queue-index">●</span>
          <span class="queue-copy"><b>${escapeHtml(current.title || 'Live radio')}</b><small>${escapeHtml([current.artist, current.album].filter(Boolean).join(' · ') || (live ? 'Live radio · on air' : 'Now playing'))}</small></span>
          <span class="queue-quality">${escapeHtml(quality(current))}</span>
          <button class="queue-delete" type="button" aria-label="Remove from queue">${icons.trash}</button>
