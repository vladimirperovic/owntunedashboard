(() => {
  ...
  const state = {
    ...
    seekDragging:false, toastTimer:null, searchTimer:null
  };
  ...
  function renderAll(){renderMode();renderConnection();renderPlayer();renderLibrary();renderRadio();}
  ...
  function renderPlayer(){
    ...
    const volume=Number(out?.volume??p.volume??0);els.volumeRange.value=volume;els.volumeRange.style.setProperty('--range-progress',`${volume}%`);els.volumeValue.textContent=`${volume}%`;
  ...
  async function playUri(uri,{shuffle=false}={}){if(!uri)return;if(state.demo){const playlist=state.playlists.find(p=>p.uri===uri),album=state.albums.find(a=>a.uri===uri);if(playlist&&isRadioPlaylist(playlist)){state.mode='radio';state.current={title:playlist.name,artist:'Live Radio',album:'Direct stream',data_kind:'url',type:'aac',bitrate:'256'};}else if(album){state.mode='music';state.current={title:album.name,artist:album.artist,album:album.name,data_kind:'file',type:'flac',bitrate:'Lossless'};}else if(playlist){state.mode='music';state.current={title:playlist.name,artist:'Playlist',album:'OwnTone',data_kind:'file',type:'flac',bitrate:'Lossless'};}state.player.state='play';renderAll();toast('Preview playback');return;}try{const qs=new URLSearchParams({uris:uri,clear:'true',playback:'start',shuffle:String(shuffle)});await request(`/queue/items/add?${qs}`,{method:'POST'});await refreshPlayback();}catch(err){toast(`Could not play: ${err.message}`);}}
  ...
  els.quickGrid.addEventListener('click',e=>{const card=e.target.closest('[data-playlist]');if(card)playPlaylistByName(card.dataset.playlist);});els.albumGrid.addEventListener('click',e=>{const card=e.target.closest('[data-uri]');if(card)playUri(card.dataset.uri);});els.playlistGrid.addEventListener('click',e=>{const card=e.target.closest('[data-uri]');if(card)playUri(card.dataset.uri);});els.radioGrid.addEventListener('click',e=>{const card=e.target.closest('[data-uri]');if(card){state.mode='radio';renderMode();playUri(card.dataset.uri);}});
  els.volumeRange.addEventListener('input',()=>{const v=Number(els.volumeRange.value);els.volumeRange.style.setProperty('--range-progress',`${v}%`);els.volumeValue.textContent=`${v}%`;});els.volumeRange.addEventListener('change',()=>setVolume(Number(els.volumeRange.value)));els.outputSelect.addEventListener('change',()=>setOutput(els.outputSelect.value));
