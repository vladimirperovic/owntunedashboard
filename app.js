(() => {
  ...
  function renderTrackChips(item,radio){if(!els.trackChips)return;const chips=item?[String(item.type||'').toUpperCase(),sampleRateText(item.samplerate),bitDepthText(item),bitrateText(item.bitrate),channelsText(item.channels),radio?'':fileSizeText(item)].filter(Boolean):[];const infoChip=item?`<button class="track-chip track-chip--info" id="trackInfoButton" type="button" title="Track details" aria-label="Track details"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" stroke-width="1.7"/><path d="M12 11v5M12 7.5h.01" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg></button>`:"";els.trackChips.innerHTML=chips.map(value=>`<span class="track-chip">${escapeHtml(value)}</span>`).join('')+infoChip;els.trackChips.hidden=!chips.length&&!item;}
