(() => {
  ...
  const svg = {
    ...
    pause:
      '<svg class="ui-icon fill" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 5h4v14H7zM14 5h4v14h-4z"/></svg>',
    home: '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m3 11 9-8 9 8"/><path d="M5 10v10h14V10M9 20v-6h6v6"/></svg>',
    search:
      '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg>',
    ...
    shuffle:
      '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h2.5c4 0 5 10 9 10H20M17 14l3 3-3 3M4 17h2.5c1.5 0 2.6-1.4 3.6-3M15.5 7H20M17 4l3 3-3 3"/></svg>',
    ...
    radio:
      '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 10h16v10H4zM7 7l9-4M8 14h.01M12 14h5M12 17h5"/></svg>',
    history:
      '<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 12a8 8 0 1 0 2.3-5.7L4 8.6V4"/><path d="M12 8v4l2.7 1.6"/></svg>',
  ...
  const initials = value => {
    const words = String(value || 'Radio')
      .replace(/^radio\s+/i, '')
      .trim()
      .split(/\s+/)
      .filter(Boolean);
  ...
  function decorateStaticUI() {
    const sideDefs = [
      ...
      [/^albums$/i, 'disc', 'library'],
      [/^playlists$/i, 'list', 'playlists'],
      [/^favorites$/i, 'heart', ''],
      [/^recently played$/i, 'history', ''],
      [/^random 500$/i, 'shuffle', ''],
      [/^morning$/i, 'sun', ''],
      [/^my music$/i, 'note', ''],
    ];
    ...
    document.querySelectorAll('.mobile-nav button').forEach(button => {
      ...
      if (/home/i.test(label)) {
        ...
        key = 'search';
        icon = 'search';
      } else if (/library/i.test(label)) {
        key = 'library';
        icon = 'disc';
      } else if (/playlists/i.test(label)) {
        key = 'playlists';
        icon = 'list';
      } else if (/radio|music/i.test(label)) {
        key = 'radio';
        icon = 'radio';
        button.id = 'mobileModeButton';
      }
      if (!key) return;
      ...
      if (button.dataset.dsDecorated !== '1') {
        const span = button.querySelector('span');
        if (span) {
          span.classList.add('mobile-nav-icon');
    ...
    if (dockHeart && dockHeart.dataset.dsDecorated !== '1') {
      dockHeart.innerHTML = svg.heart;
      dockHeart.dataset.dsDecorated = '1';
    }
    const close = document.querySelector('.search-title-row button[aria-label="Close"]');
    if (close && close.dataset.dsDecorated !== '1') {
      close.innerHTML = svg.close;
      close.dataset.dsDecorated = '1';
    }
    enhanceSearchIcons();
  }
  ...
  function syncConnectionState() {
    ...
    dot.setAttribute(
      'aria-label',
      preview ? 'Preview mode' : online ? 'OwnTone connected' : 'OwnTone offline'
    );
    ...
    document.querySelectorAll('.radio-card').forEach(card => {
      ...
      if (card.dataset.dsStationBound !== '1') {
        ...
        card.addEventListener('click', () => {
          try {
            sessionStorage.setItem('owntone-last-radio-station', name);
          } catch (_) {}
  ...
  function syncHeroStation() {
    ...
    if (!label) return;
    let station =
      document.querySelector('.radio-card.is-active .radio-station-name')?.textContent?.trim() || '';
    if (!station) {
      try {
        station = sessionStorage.getItem('owntone-last-radio-station') || '';
      } catch (_) {}
    }
  ...
  function syncMiniPlayer() {
    ...
    const playing = String(mainPlay?.getAttribute('aria-label') || '')
      .toLowerCase()
      .includes('pause');
    ...
    const generic =
      /^(choose something to play\.?|choose a station\.?|your music, beautifully simple\.?)$/i.test(title);
  ...
  function setNavActive(key) {
    document
      ...
      .forEach(button => button.classList.toggle('active', button.dataset.nav === key));
  }
  ...
  function mount() {
    ...
    if (connection)
      new MutationObserver(syncConnectionState).observe(connection, {
        subtree: true,
        childList: true,
        characterData: true,
      });
    ...
    const radioGrids = [
      document.getElementById('radioFavoritesGrid'),
      document.getElementById('radioGrid'),
    ].filter(Boolean);
    ...
    if (mobileNav)
      new MutationObserver(() => {
        ...
        relocateFolders();
        refreshNav();
      }).observe(mobileNav, { childList: true, subtree: true });
    const sideNav = document.querySelector('.side-nav');
    if (sideNav)
      new MutationObserver(enhanceDynamicNavIcons).observe(sideNav, { childList: true, subtree: true });
    ...
    if (!relocateFolders()) {
      const late = new MutationObserver(() => {
        decorateStaticUI();
        if (relocateFolders()) late.disconnect();
      });
