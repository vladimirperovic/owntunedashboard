  ...
  page.route('**/api/**', async route => {
    ...
    if (path === '/api/player')
      return route.fulfill({
        json: { state: 'play', volume: 18, item_progress_ms: 21000, item_length_ms: 240000, item_id: 'q1' },
      });
    ...
    if (path === '/api/queue' && url.searchParams.has('start'))
      return route.fulfill({ json: { count: 7, items: [] } });
    ...
    if (path === '/api/outputs/set' && method === 'PUT') {
      ...
      outputs.forEach(output => {
        output.selected = ids.includes(String(output.id));
      });
    ...
    if (path === '/api/library/albums')
      return route.fulfill({
        json: {
          items: [{ id: '123', name: 'Mezzanine', artist: 'Massive Attack', uri: 'library:album:123' }],
        },
      });
    if (path === '/api/library/albums/123/tracks')
      return route.fulfill({
        json: {
          items: [
            {
    ...
    if (path.startsWith('/api/player/volume') && method === 'PUT')
      return route.fulfill({ status: 204, body: '' });
  ...
  page.route('**/scheduler/**', route => {
    ...
    if (url.pathname.endsWith('/history'))
      return route.fulfill({
        json: {
          items: [
            { title: 'Teardrop', artist: 'Massive Attack', album: 'Mezzanine', play_uri: 'library:track:42' },
          ],
        },
      });
...
test('desktop context actions and multi-room use native OwnTone contracts', async ({ page }) => {
  ...
  await expect
    .poll(() => requests.outputVolumes.some(item => item.id === 'office' && item.volume === 31))
    .toBeTruthy();
  ...
  await page
    .locator('.album-track-row[data-context-uri]')
    .first()
