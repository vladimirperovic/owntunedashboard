...
test('now playing changes automatically with fresh data and a fade', async ({ page }) => {
    ...
    if (path === '/api/player') {
      return route.fulfill({
        json: {
          state: 'play',
          volume: 31,
          item_progress_ms: 12000,
          item_length_ms: tracks[current].length_ms,
          item_id: `q${current}`,
        },
      });
    }
    if (path === '/api/queue' && url.searchParams.get('id') === 'now_playing')
      return route.fulfill({ json: { count: 1, items: [tracks[current]] } });
    if (path === '/api/queue' && url.searchParams.has('start'))
      return route.fulfill({ json: { count: 1, items: [tracks[current]] } });
    if (path === '/api/outputs')
      return route.fulfill({
        json: {
          outputs: [
            { id: 'hp', name: 'Living Room', type: 'AirPlay', selected: true, volume: 31, format: 'alac' },
          ],
        },
      });
  ...
  await page.route('**/artwork/item/**', route => {
    if (new URL(route.request().url()).pathname.endsWith('/102'))
      return route.fulfill({ status: 204, body: '' });
    return route.fulfill({
  ...
  await page.addInitScript(() => {
    class MockOwnToneWebSocket {
      ...
      constructor() {
        ...
        setTimeout(() => {
          this.readyState = MockOwnToneWebSocket.OPEN;
          this.onopen?.();
        }, 0);
      }
      send() {}
      close() {
        this.readyState = MockOwnToneWebSocket.CLOSED;
        this.onclose?.();
      }
    }
    window.WebSocket = MockOwnToneWebSocket;
    window.__emitOwnToneEvent = payload =>
      window.__owntoneSocket?.onmessage?.({ data: JSON.stringify(payload) });
  });
...
test('live radio keeps the current station when previous or next is requested', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    return {
      before,
      after: window.OWNTONE_APP.state.current.title,
